...
def tokenize_batch(
    ...
    texts = [sample[text_key] for sample in batch]
    data = tokenizer(
        texts,
        return_tensors="pt",
        padding="max_length",
...
def save(
    ...
    booster.save_model(
        model, os.path.join(save_dir, "model"), shard=False, use_safetensors=True
    )
    running_states = {
        "epoch": epoch,
        "step": step,
...
def main():
    ...
    parser.add_argument(
        "-c", "--config", type=str, default="7b", help="Model configuration"
    )
    parser.add_argument(
        "-p",
        "--plugin",
    ...
    parser.add_argument(
        "-e", "--num_epochs", type=int, default=1, help="Number of epochs"
    )
    parser.add_argument(
        "-b", "--batch_size", type=int, default=2, help="Local batch size"
    )
    parser.add_argument("--lr", type=float, default=3e-4, help="Learning rate")
    parser.add_argument(
        "-w", "--weigth_decay", type=float, default=0.1, help="Weight decay"
    )
    parser.add_argument(
        "-s", "--warmup_steps", type=int, default=2000, help="Warmup steps"
    )
    parser.add_argument(
        "-g",
        "--grad_checkpoint",
    ...
    parser.add_argument(
        "-l", "--max_length", type=int, default=2048, help="Max sequence length"
    )
    parser.add_argument(
        "-x",
        "--mixed_precision",
    ...
    parser.add_argument(
        "-i", "--save_interval", type=int, default=1000, help="Save interval"
    )
    parser.add_argument(
        "-o", "--save_dir", type=str, default="checkpoint", help="Checkpoint directory"
    )
    ...
    parser.add_argument(
        "-a", "--flash_attention", action="store_true", help="Use Flash Attention"
    )
    parser.add_argument("--local-rank", type=str, default="0", help="rank")
    ...
    with init_ctx:
        model = LlamaForCausalLM.from_pretrained(config)
    ...
    coordinator.print_on_master(
        f"Booster init max CUDA memory: {torch.cuda.max_memory_allocated()/1024**2:.2f} MB"
    )
    coordinator.print_on_master(
    ...
    if args.load is not None:
        ...
        start_epoch, start_step, sampler_start_idx = load(
            booster, model, optimizer, lr_scheduler, args.load
        )
        coordinator.print_on_master(
            f"Loaded checkpoint {args.load} at epoch {start_epoch} step {start_step}"
        )
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        with tqdm(
            ...
            for step, batch in pbar:
                ...
                if coordinator.is_master():
                    writer.add_scalar(
                        "loss", loss.item(), epoch * num_steps_per_epoch + step
                    )
